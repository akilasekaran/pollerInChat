...
class MeetingViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        let pusher = Pusher(key: "08eccae6e2be3b921ec6",options: options)
        let channel = pusher.subscribe("chime-poller")
        channel.bind(eventName: "new_poll", callback: { (optionalData: Any?) -> Void in
            if let data = optionalData
            {
                print("Received event with data: \(data)")
                
            }})
        channel.bind(eventName: "vote", callback: { (optionalData: Any?) -> Void in
            if let data = optionalData
            {
                print("Received event with data: \(data)")
    ...
    @IBAction func votesClicked(_ sender:UIButton) {
        let url = URL(string: "https://shubham99.pythonanywhere.com/live_result")!
        ...
        let task = URLSession.shared.dataTask(with: request) { (data, response, error) in
            if let error = error {
                ...
                do {
                    let votes = try JSONSerialization.jsonObject(with: jsonData1, options:JSONSerialization.ReadingOptions(rawValue: 0)) as! [AnyObject]
                    ...
                    var vote4 : Int?
                    if votes.count>0{
                        print("JSON Dictionary! \(votes)")
                        for vote in votes {
                            let items = vote as! [String:AnyObject]
                            if items["name"] as? String == self.a1 {
                                vote1 = items["votes"] as? Int
                            }else if items["name"] as? String == self.a2 {
                                vote2 = items["votes"] as? Int
                            }else if items["name"] as? String ==  self.a3 {
                                vote3 = items["votes"] as? Int
                            }else if items["name"] as? String ==  self.a4 {
                                vote4 = items["votes"] as? Int
                            }
                            
                        }
                        DispatchQueue.main.async {
                            let storyboard: UIStoryboard = UIStoryboard(name: "Main", bundle: Bundle.main)
                            let  destVC:VotesViewController = storyboard.instantiateViewController(withIdentifier: "votes") as! VotesViewController
    ...
    @IBAction func pollButtonClicked(_ sender: UIButton) {
        
        let url = URL(string: "https://shubham99.pythonanywhere.com/get_question")!
        var request = URLRequest(url: url)
        request.httpMethod = "GET"
        var jsonData1 = Data()
        let task = URLSession.shared.dataTask(with: request) { (data, response, error) in
            if let error = error {
                ...
                do {
                    let jsonObject = try JSONSerialization.jsonObject(with: jsonData1, options:JSONSerialization.ReadingOptions(rawValue: 0))
                    guard let dictionary = jsonObject as? Dictionary<String, Any> else {
                        print("Not a Dictionary")
                        return
                    }
                    guard let options = dictionary["options"] as? [String] else {
                        return
                    }
                    if dictionary.keys.count > 0 && options.count > 0{
                        print("JSON Dictionary! \(dictionary)")
                        DispatchQueue.main.async {
                            let storyboard: UIStoryboard = UIStoryboard(name: "Main", bundle: Bundle.main)
                            var  destVC:AnswerViewController = storyboard.instantiateViewController(withIdentifier: "answer") as! AnswerViewController
                            destVC.modalPresentationStyle = UIModalPresentationStyle.overCurrentContext
                            destVC.modalTransitionStyle = UIModalTransitionStyle.crossDissolve
                            ...
                            self.a3 = options[2]
                            self.a4 = options[3]
                            self.q = dictionary["question"] as! String?
                            destVC.a1 = options[0]
                            destVC.a2 = options[1]
                            destVC.a3 = options[2]
                            destVC.a4 = options[3]
                            destVC.q = dictionary["question"] as! String?
                            self.present(destVC, animated: true, completion: nil)
                        }
                    }else {
                        DispatchQueue.main.async {
                            let storyboard: UIStoryboard = UIStoryboard(name: "Main", bundle: Bundle.main)
                            let destVC = storyboard.instantiateViewController(withIdentifier: "question") as! CreatePollController
                            destVC.modalPresentationStyle = UIModalPresentationStyle.overCurrentContext
                            destVC.modalTransitionStyle = UIModalTransitionStyle.crossDissolve
                            self.present(destVC, animated: true, completion: nil)
                        }
